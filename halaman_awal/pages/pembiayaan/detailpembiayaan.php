...
$detailDataPeminjamanNasabah = mysqli_query($koneksi, "SELECT n.*,pp.*,n.nik_username as nik_username, jp.*,jn.*,n.alamat as alamat_nasabah FROM tb_nasabah n LEFT JOIN tb_pemberian_pembiayaan_nasabah pp ON n.nik_username=pp.nik_username LEFT JOIN tb_jenis_pembiayaan jp ON jp.id_jenis_pembiayaan=pp.id_jenis_pembiayaan LEFT JOIN tb_jaminan_nasabah jn ON jn.id_pemberian_pembiayaan_nasabah=pp.id_pemberian_pembiayaan_nasabah WHERE pp.id_pemberian_pembiayaan_nasabah='$id' AND n.nik_username='$nik_username' ORDER BY id_jaminan_nasabah DESC");
?>
